...
export function padFips(value: string | number, width = 5): string {
  const s = String(value).replace(/\.0$/, "");
  return s.padStart(width, "0");
}

export function stateOfFips(fips: string): string | undefined {
  return STATE_FIPS_TO_ABBR[fips.slice(0, 2)];
}

export const ALL_STATE_ABBRS = Object.values(STATE_FIPS_TO_ABBR).sort();
